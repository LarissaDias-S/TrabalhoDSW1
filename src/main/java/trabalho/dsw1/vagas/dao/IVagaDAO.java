...
public interface IVagaDAO extends CrudRepository<Vaga, Long>  {
    ...
    @Query("SELECT v FROM Vaga v WHERE v.status = 'Aberta' AND v.localizacao = :cidade")
    ...
    @Query("SELECT v FROM Vaga v WHERE v.status = 'Aberta' AND v.localizacao = :cidade AND v.id NOT IN (SELECT c.vaga.id FROM Candidato c WHERE c.profissional.id = :profissionalId) AND v.dataLimite >= :dataHoje")
    List<Vaga> findVagasAbertasPorCidade(@Param("cidade") String cidade, Long profissionalId, LocalDate dataHoje);

    @Query("SELECT v FROM Vaga v WHERE v.status = 'Aberta' AND v.localizacao = :cidade AND v.dataLimite >= :dataHoje")
